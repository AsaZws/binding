	...
	Plate.prototype = {
		...
		initHtml: function () {
			...
			var html = '<p>请输入车牌</p><div class="plate">';
				...
				html += '</div>'
			this._plate.innerHTML = html;
			// 获取车牌框
			this.plateFrame = this._plate.getElementsByClassName("plate-frame")[0];
			// 将初始车牌的值渲染到车牌框
			this.plateFrame.innerHTML = placeStrs(PNUM);
			// 获取车牌框li
			this.plateFrameLi = this.plateFrame.getElementsByTagName("li");
			 // 获取键盘
			this.placeLetter = this._plate.getElementsByClassName('place-letter')[0];
			// 渲染完成获取键盘
			...
			if(this.plateIndex == 0) {
				this.placeLetter.innerHTML = placeStrs(PVS);
			} else {
				this.placeLetter.innerHTML = placeStrs(NUM);
			}
			// 获取关闭
			this.shutDown = this._plate.getElementsByClassName('shut-down')[0];
			// 查询按钮
			this.queryDetail = this._plate.getElementsByClassName('queryDetail')[0];
